...
// 3. RUTA PRINCIPAL - EL SECRETO ESTÁ EN LAS COMILLAS DOBLES
app.get('/', async (req, res) => {
  try {
    // Usamos comillas dobles para "ID" y los nombres con espacios
    const sql = `
      SELECT 
        "ID", 
        "FECHA REGISTRO", 
    ...
    let filas = cargas.map(c => {
      // Buscamos el flete usando el ID exacto (en mayúsculas)
      ...
      return `
        <tr style="border-bottom: 1px solid #334155">
          ...
          <td><span style="background:${f?.est_pago === 'PAGADO' ? '#065f46' : '#7f1d1d'}; padding:4px 8px; border-radius:5px">
    ...
    res.status(500).send("ERROR DE IDENTIFICADOR: " + err.message);
  }
});
  ...
  res.send(`
    <body style="background:#0f172a; color:#f1f5f9; font-family:sans-serif; padding:50px">
      <form action="/guardar/${req.params.id}" method="POST" style="background:#1e293b; padding:20px; border-radius:10px; max-width:300px; margin:auto">
        <h3>Editar Servicio #${req.params.id}</h3>
...
db.sync().then(() => app.listen(PORT, () => console.log('🚀 Sincronizado con ID en mayúsculas')));

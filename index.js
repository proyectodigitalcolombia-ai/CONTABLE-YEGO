...
app.post('/api/update-cell', async (req, res) => {
  try {
    ...
    if (['v_flete', 'v_facturar', 'valor_descuento', 'saldo_a_pagar', 'retefuente', 'reteica', 'valor_anticipo', 'sobre_anticipo'].includes(campo)) {
      valorProcesado = parseFloat(valorProcesado) || 0;
    }

    await registro.update({ [campo]: valorProcesado.toUpperCase() });
...
app.get('/', async (req, res) => {
  try {
    ...
    const filas = cargas.map(c => {
      ...
      // Información directa de LOGISV20
      const fleteLogis = Number(c.v_flete || 0);
      const facturarLogis = Number(c.v_facturar || 0);
      
      if((f.est_pago || 'PENDIENTE') === 'PENDIENTE') totalPendiente += fleteLogis;
      ...
      return `
        <tr class="fila" data-placa="${(c.placa || '').toLowerCase()}" style="border-bottom: 1px solid #334155; font-size: 11px;">
          ...
          <td style="${tdBase} color: #10b981; font-weight: bold; background: rgba(16, 185, 129, 0.05);">$${fleteLogis.toLocaleString('es-CO')}</td>
          <td style="${tdBase} color: #3b82f6; font-weight: bold; background: rgba(59, 130, 246, 0.05);">$${facturarLogis.toLocaleString('es-CO')}</td>
          ...
          ${excelCell('dias_sin_cumplir', f.dias_sin_cumplir)}
          ...
          <table style="width:100%; border-collapse:collapse; background:#1e293b; min-width:6500px;">
            <thead style="background:#1e40af; color:white; font-size:10px; text-transform:uppercase;">
              <tr>
                ...
                <th style="background:#065f46;">VALOR FLETE A PAGAR (LOGIS)</th><th style="background:#1e3a8a;">VALOR FLETE A FACTURAR (LOGIS)</th>

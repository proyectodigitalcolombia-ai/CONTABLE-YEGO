...
// --- MODELO CON MAPEO DE MAYÚSCULAS ---
const Carga = db.define('Carga', {
  id: { type: DataTypes.INTEGER, primaryKey: true },
  placa: { type: DataTypes.STRING },
  cont: { type: DataTypes.STRING },
  // Forzamos el nombre de la columna a mayúsculas como se ve en tu imagen
...
const css = `<style>
  ...
  .badge{background:#334155; padding:2px 6px; border-radius:4px; color:#cbd5e1;}
</style>`;
...
app.get('/', async (req, res) => {
  try {
    ...
    res.send(`<html><head><title>YEGO MASTER</title>${css}</head><body><div class="container">
      <h1 style="color:#3b82f6">YEGO 💰 Finanzas <small style="color:#64748b; font-size:12px">| Control Total de Cargas</small></h1>
    ...
    res.send(`<h2>Error de Sincronización</h2><p>${err.message}</p><p>Verifica que los nombres de las columnas en la tabla Cargas coincidan.</p>`); 
  }
});
